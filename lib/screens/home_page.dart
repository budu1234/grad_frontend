...
class _HomePageState extends State<HomePage> {
  ...
  void initState() {
    super.initState();
    fetchUser();
    _futureSchedule = fetchSchedule(selectedDate);
    _futureCompleted = fetchCompleted(selectedDate);
  }
  ...
  Future<Map<String, dynamic>> fetchSchedule(DateTime date) async {
    ...
    final response = await http.get(
      Uri.parse('http://10.0.2.2:5000/schedule/?date=$dateStr'),
      headers: {'Authorization': 'Bearer ${widget.jwtToken}'},
    );
    if (response.statusCode == 200) {
  ...
  Future<List<ScheduledTask>> fetchCompleted(DateTime date) async {
    final dateStr = DateFormat('yyyy-MM-dd').format(date);
    final response = await http.get(
      Uri.parse('http://10.0.2.2:5000/schedule/completed?date=$dateStr'),
      headers: {'Authorization': 'Bearer ${widget.jwtToken}'},
    );
    if (response.statusCode == 200) {
      final data = jsonDecode(response.body);
  ...
  Widget build(BuildContext context) {
    final todayStr = DateFormat('d MMM yyyy').format(selectedDate);
    ...
    return Scaffold(
      backgroundColor: Colors.white,
drawer: CustomDrawer(jwtToken: widget.jwtToken, currentPage: DrawerPage.checklist),      appBar: AppBar(
        backgroundColor: Colors.white,
        elevation: 0,
        leading: Builder(
      ...
      body: Stack(
        children: [
          ...
          Positioned(
            top: 0,
            right: 0,
            child: Image.asset('assets/img/Ellipse_4.png', fit: BoxFit.cover),
          ),
          Positioned(
            bottom: 0,
            left: 0,
            child: Image.asset('assets/img/Ellipse_5.png', fit: BoxFit.cover),
          ),
          Padding(
            ...
            child: ListView(
              children: [
                ...
                // Header with Today and calendar arrows
                Row(
                  children: [
                    const Text(
                      "Today",
                  ...
                  builder: (context, snapshot) {
                    ...
                    return Column(
                      children: [
                        ...
                        FutureBuilder<List<ScheduledTask>>(
